/**
 * @file fiddling.h
 * @brief Miscellaneous utilities, including hexdump.
 */
...
/**
 * @brief Theme configuration for hexdump colors.
 *
...
typedef struct {
  ...
  const char *reset;         /**< Reset escape code (e.g., `\x1b[0m`) . */
} hexdump_theme;
...
/**
 * @brief Configuration options for the hexdump function.
 */
typedef struct {
...
/**
 * @brief Performs a hexdump of the provided data with various options.
 *
 ...
 *
 * @param data The data to dump. Must not be `NULL` .
 * @param size The size of the data in bytes.
 * @param options Pointer to options struct for customization. If `NULL` ,
 *                `HEXDUMP_DEFAULT_OPTIONS` will be used.
 */
